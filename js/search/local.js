window.onload = () => {
    ...
    function initLunr() {
        const configuredPath = GLOBAL_CONFIG?.localsearch?.path
        const root = (GLOBAL_CONFIG.root || '/').replace(/\/?$/, '/')
        const searchPath = !configuredPath
            ? `${root}search.xml`
            : (/^https?:\/\//i.test(configuredPath)
                ? configuredPath
                : `${root}${configuredPath.replace(/^\//, '')}`)
